...
// We create the channels first without any IP addressing information
  ...
  p2p.SetDeviceAttribute ("DataRate", StringValue ("120Mbps"));//1.2Mbps
  ...
  p2p.SetDeviceAttribute ("DataRate", StringValue ("1.2Mbps"));//1.2Mbp
  p2p.SetChannelAttribute ("Delay", StringValue ("8ms"));
...
// Create router nodes, initialize routing database and set up the routing
 ...
 // Create BuldSend Application to send Tcp packets  //
  ...
  oo.SetAttribute ("PacketSize", UintegerValue (500));
  oo.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=0.8]"));
  oo.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0.2]"));
  oo.SetAttribute ("DataRate", DataRateValue (DataRate ("1500kb/s")));

  ApplicationContainer SourceApp = oo.Install (host.Get (0));
  SourceApp.Start (Seconds (0.0));
  SourceApp.Stop (Seconds (15.0));



    p2p.EnablePcapAll("mptcp-simulation", true);
